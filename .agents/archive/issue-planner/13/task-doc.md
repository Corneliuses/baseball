...
- [x] Add `replyTo?: string` to `SendEmailInput` in `src/lib/email.ts`, passed through to
      `resend.emails.send` (Resend's field is `replyTo`); omit when absent
- [x] Add `src/lib/email.test.ts` covering the pass-through and the omission
- [x] Create `src/lib/messages.ts`:
...
- [x] Write `src/lib/messages.test.ts` — the full matrix from design-doc.md's Testing
      Strategy, including the parent-can-never-reach-a-parent invariant
- [x] Create `src/emails/team-message-email.ts` — pure `buildTeamMessageEmail({ teamName,
      subject, teamId, env })` → `{ subject: "[<teamName>] <subject>", teamUrl }`,
      mirroring `src/emails/added-to-team-email.ts`
- [x] Write `src/emails/team-message-email.test.ts`
- [x] Create `src/emails/TeamMessageEmail.tsx` — plain React Email template in the
      `InvitationEmail.tsx` style: sender name + team name, `pre-wrap` body, link to the
      team home; no images

## Phase 2: Server action, pages, nav

- [x] Create `src/app/t/[teamId]/messages/new/actions.ts` — `sendTeamMessageAction`:
...
- [x] Write `src/app/t/[teamId]/messages/new/actions.test.ts` (pattern:
      `roster/invite/actions.test.ts`)
- [x] Create `src/app/t/[teamId]/messages/new/page.tsx` — compose form;
...
- [x] Write `src/app/t/[teamId]/messages/new/page.test.tsx`
- [x] Create `src/app/t/[teamId]/messages/page.tsx` — `requireTeamAccess` read; PARENT →
      `redirect` to `messages/new`; COACH+ → broadcast history via `listMessages` +
      "New message" link
- [x] Write `src/app/t/[teamId]/messages/page.test.tsx`
- [x] Add `{ href: `${base}/messages`, label: "Messages" }` to the base items in
      `src/components/TeamNav.tsx` (all roles)
- [x] Update `src/components/TeamNav.test.tsx` for the new tab

## Pre-Commit Gate

Per AGENTS.md `## Commands`:

- [x] `pnpm check` (lint → typecheck → test) ✅
- [x] `pnpm exec next build` ✅ (bare `pnpm build` needs `DATABASE_URL` for
